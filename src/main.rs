...
impl FromStr for ReportLine {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        ...
        let naive = split[0].parse::<NaiveDateTime>().expect("Could not parse date");
...
fn get_points_from_report_lines<'a>(lines: Vec<ReportLine>) -> (Vec<(f32, f32)>, Vec<(f32, f32)>) {
    ...
    lines
        .into_iter()
        .map(|line| { (to_point(&min, &line.date, &(line.closed)), to_point(&min, &line.date, &(line.open + line.closed))) })
        .unzip()
}

fn main() {
    let args = Cli::parse();
    let entries = std::fs::read_to_string(args.path).unwrap().lines().map(|l| { ReportLine::from_str(l).unwrap() }).collect::<Vec<ReportLine>>();
